...
export function DrawStrip({ draws, planned = 0 }: DrawStripProps) {
  const byDraw = new Map(draws.map((record) => [record.draw, record.outcome]))
  const length = Math.max(planned, ...draws.map((record) => record.draw), 0)
  const counts = drawCounts(draws)
  const summary = ORDER.filter((outcome) => counts[outcome] > 0)
    .map((outcome) => `${formatCount(counts[outcome])} ${OUTCOME_WORDS[outcome]}`)
    .join(', ')
  const remaining = Math.max(0, length - draws.length)
  return (
    <figure className="space-y-2">
      <ol className="flex flex-wrap gap-0.5" aria-label="Routing-dilution draws, in order">
        {Array.from({ length }, (_, index) => {
          ...
          const cell = outcome === undefined ? undefined : CELLS[outcome]
          return (
            <li
              key={draw}
              title={`Draw ${draw}: ${outcome === undefined ? 'not yet drawn' : outcome}`}
              aria-label={`Draw ${draw}: ${outcome === undefined ? 'not yet drawn' : outcome}`}
